...
const SUPABASE_URL = 'https://fqdetuahysdsfjcflwgh.supabase.co';
const SUPABASE_KEY = 'sb_publishable_hlcNO-vjk255eTaUnpWLIQ_hHTCOnJS';

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// Dynamic user system with LocalStorage (with exception safety)
let registeredUsers = [];
try {
  registeredUsers = JSON.parse(localStorage.getItem('sm_app_users')) || [];
...
try {
  currentUser = JSON.parse(localStorage.getItem('sm_current_user')) || null;
} catch (e) {
  currentUser = null;
}
...
// Secure User Sync controllers for Admin and Operator accounts
async function fetchUsers() {
  try {
    ...
    if (data && data.length > 0) {
      registeredUsers = data.map(normalizeUser);
      localStorage.setItem('sm_app_users', JSON.stringify(registeredUsers));
    }
  } catch (e) {
    console.warn("Could not load users from Supabase (maybe app_users table does not exist). Falling back to LocalStorage.", e);
  }
}
...
async function saveUserToDb(user) {
  try {
    ...
    if (error) {
      console.warn("Snake case user insert failed, trying camelCase format", error);
      const dbUserCamel = {
        id: user.id,
    ...
    console.warn("Could not save user to Supabase (table app_users may not exist). Saved locally.", e);
  }
}
...
async function deleteUserFromDb(email) {
  try {
    ...
    console.warn("Could not delete user from Supabase app_users table.", e);
  }
}
...
function updateAuthUI() {
  ...
  if (currentUser) {
    ...
    const exists = registeredUsers.find(u => u && u.email && currentUser && currentUser.email && u.email.toLowerCase() === currentUser.email.toLowerCase());
...
window.handleLoginSubmit = async function(event) {
  ...
  // Try to sync credentials from Supabase before validation
  try {
    ...
    if (!error && dbUser) {
      const mapped = normalizeUser(dbUser);
      const idx = registeredUsers.findIndex(u => u && u.email && u.email.toLowerCase() === email);
      if (idx !== -1) {
        registeredUsers[idx] = mapped;
      } else {
        registeredUsers.push(mapped);
      }
      localStorage.setItem('sm_app_users', JSON.stringify(registeredUsers));
    }
  } catch (err) {
    console.warn("Could not sync user credentials from cloud database, using local cache", err);
  }
  
  const user = registeredUsers.find(u => u && u.email && u.email.toLowerCase() === email);
  if (!user) {
...
window.handleRegisterSubmit = async function(event) {
  ...
  const exists = registeredUsers.some(u => u && u.email && u.email.toLowerCase() === email);
  if (exists) {
  ...
  // Save user details securely in Supabase Database - Run in background, non-blocking
...
function renderUsersList() {
  ...
  registeredUsers.forEach(user => {
    ...
    const isCurrent = currentUser && user.email && currentUser.email && user.email.toLowerCase() === currentUser.email.toLowerCase();
    ...
    tr.innerHTML = `
      ...
      <td class="px-4 py-3 pb-3.5 font-mono text-[11px] text-slate-355">${user.employeeId}</td>
...
window.openEditUserModal = function(email) {
  const user = registeredUsers.find(u => u && u.email && email && u.email.toLowerCase() === email.toLowerCase());
  if (!user) return;
...
window.copySchemaSql = function() {
  const queryBlock = document.getElementById('sqlQueryBlock');
  if (queryBlock) {
    navigator.clipboard.writeText(queryBlock.innerText.trim()).then(() => {
      showToast("Schema SQL copied to clipboard successfully!", "success");
    }).catch(err => {
      showToast("Failed to copy schema action automatically. Please select and copy manually.", "error");
    });
  }
};
...
window.handleAdminUserSubmit = async function(event) {
  if (event) event.preventDefault();
  ...
  // Duplication check
  const duplicate = registeredUsers.some(u => u && u.email && u.email.toLowerCase() === email && (mode === 'create' || u.email.toLowerCase() !== oldEmail.toLowerCase()));
  if (duplicate) {
  ...
  if (mode === 'create') {
    ...
    registeredUsers.push(newUser);
    saveUserToDb(newUser); // Run in background, non-blocking
    showToast(`Registered system operator profile for ${fullName}.`, "success");
  } else {
    // Editing existing operator
    const user = registeredUsers.find(u => u && u.email && u.email.toLowerCase() === oldEmail.toLowerCase());
    if (user) {
      ...
      if (currentUser && currentUser.email && oldEmail && currentUser.email.toLowerCase() === oldEmail.toLowerCase()) {
        currentUser = { ...currentUser, ...user };
        localStorage.setItem('sm_current_user', JSON.stringify(currentUser));
      }
      saveUserToDb(user); // Run in background, non-blocking
...
window.toggleDisableUser = async function(email) {
  if (currentUser && currentUser.email && email && currentUser.email.toLowerCase() === email.toLowerCase()) {
...
window.deleteUser = async function(email) {
  ...
  if (!isAdmin) {
    showToast("AccessViolation: Permission denied. User deletion is reserved for Administrators.", "error");
    return;
  }
  ...
  if (confirm(`CRITICAL SYSTEM AUDITING ACTION: Do you want to completely erase user account for ${user.fullName}? All logging signatures remains trace offline.`)) {
    registeredUsers = registeredUsers.filter(u => u && u.email && email && u.email.toLowerCase() !== email.toLowerCase());
    localStorage.setItem('sm_app_users', JSON.stringify(registeredUsers));
    
    deleteUserFromDb(email); // Run in background, non-blocking
...
async function initApp() {
  ...
  // Render views with existing local/cached records instantly
  updateAuthUI();
  renderDashboard();
  setupAutocompletes();
  
  // Synchronously seed standard default users array in local storage if completely empty
  if (registeredUsers.length === 0) {
    const defaultManager = {
      ...
      employeeId: 'SGS-001',
      role: 'Admin',
      email: 'ramesh.k@syrmasgs.com',
      password: 'password123',
      disabled: false,
      avatar: 'R',
      avatarColor: 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400 font-extrabold shadow-[inset_0_0_8px_rgba(6,182,212,0.15)]'
    };
    registeredUsers.push(defaultManager);
    localStorage.setItem('sm_app_users', JSON.stringify(registeredUsers));
    updateAuthUI();
  }
  
  // Fetch newest synchronized records from Supabase in background
  try {
    const { data, error } = await supabase
      .from('app_users')
      .select('*');
    ...
    console.warn("Could not load users from Supabase in background:", err);
  }
  ...
  try {
    ...
    console.warn("Could not load equipment from Supabase in background:", err);
  }
}

// Setup Event Listeners
function setupEventListeners() {
  // Fix Auth tab toggles
  ...
  window.addEventListener('click', (e) => {
    ...
    const sqlSchemaModal = document.getElementById('sqlSchemaModal');
    if (e.target === detailModal) closeDetailModal();
    if (e.target === printModal) closePrintModal();
    if (e.target === adminUserModal) window.closeAdminUserModal();
    if (e.target === sqlSchemaModal) window.closeSchemaModal();
  });
}

// File Attachment System with Supabase Storage
async function handleUploadedFiles(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    showToast(`Uploading ${file.name} to storage...`, 'info');
    
    try {
      const fileExt = file.name.split('.').pop();
      const uniqueFileName = `${Date.now()}-${Math.random().toString(36).substring(2, 10)}.${fileExt}`;
      const filePath = `calibrations/${uniqueFileName}`;
      ...
      console.warn("Supabase Storage bucket upload failed, falling back to local file metadata.", err);
      uploadedFilesCache.push({
        name: file.name,
        type: file.type,
        size: file.size,
      ...
      showToast(`Bucket missing or offline. Attachment metadata registered locally.`, 'warning');
    }
  }
...
window.removeUploadedFile = async function(index) {
  const file = uploadedFilesCache[index];
  if (file && file.path) {
    try {
      showToast(`Removing ${file.name} from Supabase storage...`, 'info');
  ...
  filtered.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td class="px-4 py-3 text-slate-200">
        ...
        <div class="text-xs text-slate-505 text-slate-500">${item.department}</div>
      </td>
...
window.deleteEquipment = async function(assetNo) {
  ...
  if (confirm(`Are you absolutely sure you want to remove Asset [${assetNo}]?`)) {
    showToast("Deleting asset...", "info");
    
    // Optimistically update local memory list for robust UX
    equipmentList = equipmentList.filter(e => e.assetNumber !== assetNo);
    renderDashboard();
    ...
    try {
      ...
      if (error) {
        ...
        if (errorCamel) {
          console.error("Database deletion failed:", errorCamel.message);
        }
      }
    } catch (e) {
      console.warn("Could not synchronize deletion with Supabase. Done locally.", e);
    }
    
    showToast(`Asset [${assetNo}] removed permanently.`, "success");
    
    // Re-fetch to ensure sync with remote DB state
    try {
      await fetchEquipment();
      renderDashboard();
    } catch (err) {
      console.warn("Follow-up fetch skipped or offline", err);
    }
  }
};
    ...
    if (item.documents && item.documents.length > 0) {
      item.documents.forEach((doc, idx) => {
        ...
        docDiv.innerHTML = `
          ...
          <button type="button" onclick="previewDoc('${doc.name}', '${doc.type}', '${doc.url || ''}')" class="px-2.5 py-1 text-xs shrink-0 bg-cyan-500/10 hover:bg-cyan-500 hover:text-white transition-colors duration-200 text-cyan-400 rounded-md font-semibold cursor-pointer">
      ...
      docList.innerHTML = `
        <div class="col-span-2 text-center text-xs text-slate-505 text-slate-500 py-3 bg-slate-900/40 border border-slate-800/80 rounded-lg">
...
window.previewDoc = function(name, type, url = '') {
  ...
  header.innerHTML = `
    ...
    <button onclick="document.getElementById('docPreviewFrameContainer').classList.add('hidden')" class="text-slate-550 hover:text-white text-xs cursor-pointer"><i class="fa-solid fa-times"></i> Dismiss Sandbox</button>
  `;
  ...
  if (url) {
    if (isPdf) {
      docVisualDiv.innerHTML = `
        <div class="py-10 max-w-sm flex flex-col items-center gap-3">
          <i class="fa-solid fa-file-pdf text-rose-500 text-5xl animate-pulse"></i>
          <div class="text-slate-200 text-sm font-semibold">${name}</div>
          <p class="text-xs text-slate-500">Accredited laboratory calibration report hosted securely on Supabase Storage.</p>
          <a href="${url}" target="_blank" class="px-4 py-2 bg-rose-500/10 hover:bg-rose-500 hover:text-white border border-rose-500/25 transition-all duration-200 text-rose-300 rounded font-semibold text-xs flex items-center gap-1.5 focus:outline-none">
            <i class="fa-solid fa-file-arrow-down mr-1"></i> Download / View PDF Certificate
          </a>
        </div>
      `;
    } else {
      docVisualDiv.innerHTML = `
        <div class="p-3 w-full flex flex-col items-center justify-center gap-2">
          <div class="text-xs text-slate-500 mb-2 uppercase font-mono tracking-wide">REAL IMAGE RETRIEVED FROM SUPABASE STORAGE</div>
          <div class="relative max-w-full max-h-64 border border-cyan-500/20 rounded bg-slate-900 flex items-center justify-center overflow-hidden">
            <img src="${url}" alt="${name}" class="max-h-60 object-contain rounded shadow-lg" referrerPolicy="no-referrer" />
          </div>
          <a href="${url}" target="_blank" class="mt-2 text-cyan-400 hover:underline text-xs"><i class="fa-solid fa-up-right-from-square mr-1"></i>Open full size image</a>
        </div>
      `;
    }
  } else {
    if (isPdf) {
      ...
      docVisualDiv.innerHTML = `
        <div class="p-3 w-full flex flex-col items-center justify-center gap-2">
          <div class="text-xs text-slate-550 mb-1 max-w-xs uppercase font-mono tracking-wide">IMAGE RETRIEVED FROM CALIBRATION ARCHIVES</div>
          <div class="relative w-48 h-48 border border-cyan-500/20 rounded bg-slate-900 flex items-center justify-center overflow-hidden">
            ...
            <div class="relative text-xs text-center px-4 font-sans text-slate-350 select-none">
